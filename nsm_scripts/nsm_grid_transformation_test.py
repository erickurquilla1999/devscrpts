...
# !! WARNING POSSIBLE SOURCE OF ERROR !!
print(f"dump_file['startx[1]'][0] = {dump_file['startx[1]'][0]}")
print(f'np.min(X1_harm_coordinates) = {np.min(X1_harm_coordinates)}')
# !! WARNING POSSIBLE SOURCE OF ERROR !!
...
J      = poly_norm * y_tilde * ( 1  + (   y_tilde / poly_xt )**poly_alpha ) / ( poly_alpha + 1 )  + 0.5 * np.pi 
